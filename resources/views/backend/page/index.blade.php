            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <img class="img-preview" src="{{asset($record->logo) }}" alt="">
                    </td>
                    <td>{{$record->brand}}</td>
                    <td>{{$record->title}}</td>
                    <td>{{date('d-m-Y H:i:s',strtotime($record->created_at))}}</td>
                    <td class="text-center">
                        <a href="{{route('admin.page.edit', $record->id)}}" title="{!! trans('base.edit') !!}" class="success"><i class="icon-pencil"></i></a>
                        <form action="{!! route('admin.page.destroy', ['id' => $record->id]) !!}" method="POST" style="display: inline-block">
                            {!! method_field('DELETE') !!}
                            {!! csrf_field() !!}
